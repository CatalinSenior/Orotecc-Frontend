...
const Step1SetPassword = ({ onNext, onSignIn }: Step1SetPasswordProps) => (
  ...
  >
    ...
    <div>
      <h4 className="text-lg font-semibold text-[#222] mb-1">Create your password</h4>
      <p className="text-sm text-[#686e7b] mb-4">Your password provides you with sign in access to Orotecc, so it&apos;s important we get it right.</p>
    </div>
    <div className="flex flex-col gap-6">
      <div>
        <label className="block text-base font-semibold text-[#222] mb-1">Root user password</label>
        <input
          type="password"
      ...
      <div>
        <label className="block text-base font-semibold text-[#222] mb-1">Confirm root user password</label>
        <input
          type="password"
          className="w-full mt-0.5 p-3 border border-[#e0e0e0] rounded-lg text-base focus:outline-none focus:border-[#6a4cff]"
          placeholder=""
    ...
    <div className="flex items-center text-center my-4 text-[#686E7B] text-sm">
      ...
      <span className="px-6 z-10">OR</span>
      <div className="flex-1 h-px bg-[#686E7B]" />
    </div>
    <button
      type="button"
